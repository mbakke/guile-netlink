  ...
  (let ((inactive-time (get-attr sta-info NL80211_STA_INFO_INACTIVE_TIME))
    ...
    (when sta-flags
      (let ((mask (nl80211-sta-flag-update-attr-type-mask sta-flags))
        ...
        (when (logtest mask authorized)
          (format #t "\tauthorized:\t~a~%"
                  (if (logtest set authorized)
                      "yes" "no")))
        (when (logtest mask authenticated)
          (format #t "\tauthenticated:\t~a~%"
                  (if (logtest set authenticated)
                      "yes" "no")))
        (when (logtest mask authorized)
          (format #t "\tassociated:\t~a~%"
                  (if (logtest set associated)
                      "yes" "no")))
        (when (logtest mask preamble)
          (format #t "\tpreamble:\t~a~%"
                  (if (logtest set preamble)
                      "short" "long")))
        (when (logtest mask wme)
          (format #t "\tWMM/WME:\t~a~%"
                  (if (logtest set wme)
                      "yes" "no")))
        (when (logtest mask mfp)
          (format #t "\tMFP:\t\t~a~%"
                  (if (logtest set mfp)
                      "yes" "no")))
        (when (logtest mask tdls-peer)
          (format #t "\tTDLS peer:\t~a~%"
                  (if (logtest set tdls-peer)
                      "yes" "no")))))
    (when bss-param
      (let ((dtim-period
